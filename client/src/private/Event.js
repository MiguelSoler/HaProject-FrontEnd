...
const Event = () => {
  ...
  const handleLoader = () => dispatch({ type: 'showModal', modalType: 'loader' })
  ...
  const handleSubmit = async (e) => {
    ...
    setError(false)
    try {
      const ret = await fetch('http://localhost:3000/loader', {
        method: 'POST',
        body: JSON.stringify(),
        headers: {
      ...
      const {token, user} = await ret.json()
  ...
  return (   
      <form className="login-form" onSubmit={handleSubmit}>
        ...
        <div className="buttonsContainer">
          <button><Link to='/private/loader' className="registrate" onCLick={handleLoader}>Crear Carrera</Link></button>
